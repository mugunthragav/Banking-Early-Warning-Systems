...
router = APIRouter(
    prefix="",  # Changed from "/" to "" to avoid AssertionError
    tags=["Market Risk"]
)

# Load model results
model_file_path = project_root / 'models' / 'all_commodities_model.json'
try:
...
# Symbol descriptions
symbol_descriptions = {
    '@CL#C': 'Energy', '@C#C': 'Agriculture', '@ES': 'Equity Index', '@NG': 'Energy',
    '@SI': 'Metals', '@TY': 'Interest Rate', '@AD': 'Currencies', '@EC': 'Currencies',
    '@6E': 'Currencies', '@GC': 'Metals', '@LE': 'Equity Index', '@NQ': 'Equity Index',
    '@SB': 'Equity Index', '@YM': 'Equity Index'
}
    ...
    try:
        ...
        if not heatmap_dict:
            return go.Figure().update_layout(
                title=f'Risk Exposure Heat Map for {symbol} ({symbol_descriptions.get(symbol, "Unknown Symbol")})<br>(Data Not Available)'
            ), pd.DataFrame()
        ...
        return go.Figure().update_layout(
            title=f'Risk Exposure Heat Map for {symbol} ({symbol_descriptions.get(symbol, "Unknown Symbol")})<br>(Error Loading Data: {e})'
        ), pd.DataFrame()
...
async def get_metrics(symbol: str):
    try:
        ...
        feature_importance = [{"feature": k, "importance": float(v)} for k, v in mdi_imp_data.items()]
        ...
        var_metrics = [{
            ...
            "capital_impact": float(row['capital_impact']),
            "liquidity_impact": float(row['liquidity_impact'])
        } for row in var_df.reset_index().to_dict('records')]
        ...
        stress_metrics = [{
            ...
            "margin_shortfall": float(row['margin_shortfall']),
            "revenue_impact": float(row['revenue_impact']),
            "capital_impact": float(row['capital_impact']),
            "liquidity_impact": float(row['liquidity_impact'])
        } for row in stress_df.reset_index().to_dict('records')]
        ...
        } for row in backtest_df.reset_index().to_dict('records')]
...
@router.get("/visualizations/{symbol}", response_model=Dict[str, Any])
async def get_visualizations(symbol: str):
    try:
        ...
        if not mc_var_values_dict or all(v == 0 for v in mc_var_values):
...
async def get_heatmap(symbol: str):
    try:
        ...
        heatmap_data_list = [
            {"scenario_metric": idx, "value": float(row['Value'])} for idx, row in heatmap_data.iterrows()
        ] if not heatmap_data.empty else []
...
@router.get("/report/{symbol}", response_class=FileResponse)
async def get_report(symbol: str):
    try:
        ...
        report_path = report_dir / f"{symbol}_report.pdf"

        try:
            report.generate_report(
                {symbol: results[symbol]},
                {'report': {'output_dir': str(report_dir), 'format': 'pdf'}}
            )
        except AttributeError as ae:
            logger.error(f"Report module does not have 'generate_report' method: {ae}")
            raise HTTPException(status_code=500, detail="Report generation module is misconfigured")
        except Exception as re:
            logger.error(f"Report generation failed: {re}")
            raise HTTPException(status_code=500, detail=f"Report generation failed: {str(re)}")
